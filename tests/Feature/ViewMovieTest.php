...
class ViewMovieTest extends TestCase
{
    ...
    {
        // The Http facade's fake method allows you to instruct the HTTP client to return 
        // stubbed / dummy responses when requests are made.
        Http::fake([
            'https://api.themoviedb.org/3/movie/popular' => $this->fakePopularMovies(),
            'https://api.themoviedb.org/3/movie/now_playing' => $this->fakePlayingMovies(),
            'https://api.themoviedb.org/3/movie/list' => $this->fakeGenres(),
        ]);
        // examining the responses of index page
        $res = $this->get(route('movies.index'));
        
        // to see if the response is successful
        $res->assertSuccessful();
        // we need to use phpunit to test the api 
        // if this doesn't work, use assertSee then I went 
        // to MovieController and dd(popularMovies) 
        // then run ./vendor/bin/phpunit in your terminal
        $res->assertSee('Popular Movies');
        $res->assertSee('Fake Movie');
        $res->assertSee('Adventure, Drama, Mystery, Science Fiction, Thriller');
        $res->assertSee('Now Playing');
        $res->assertSee('Now Playing Fake Movies');
    }
    ...
    {
        Http::fake([
            // using * is to match any id based on the $id in MovieController 
            'https://api.themoviedb.org/3/movie/*' => $this->fakeSingleMovie(),
        ]);

        $res = $this->get(route('movies.show', 12345));
        $res->assertSee('Fake Jumanji');
        $res->assertSee('John Doe');
        $res->assertSee('Casting Director');
        $res->assertSee('The Rock');
    }
    ...
    private function fakePlayingMovies()
    {
        return Http::response([
            'results' => [
                [
